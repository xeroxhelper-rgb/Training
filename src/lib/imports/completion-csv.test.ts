...
describe("parseCompletionCsv", () => {
  ...
  it("also accepts headerless four-column files", () => {
    const [row] = parseCompletionCsv("2020031234,홍길동,2026-09-01,수료\n");
    expect(row).toMatchObject({ employeeNumber: 2020031234, name: "홍길동", status: "completed", error: null });
  });
});
